...
return view.extend({
    ...
    render: function() {
        ...
        // --- ATF Status Indicator ---
        o = s.option(form.DummyValue, '_atf_status', _('Current ATF State'));
        o.rawhtml = true;
        o.cfgvalue = function(section_id) {
            let atf = uci.get('advanced', section_id, 'atf_enable');
            let watf = uci.get('advanced', section_id, 'hw_atf_enable');

            if (atf === '1' && watf === '1') {
                return '<span class="label success">' + _('Active (ATF + WATF)') + '</span>';
            } else if (atf === '1') {
                return '<span class="label success">' + _('Active (ATF)') + '</span>';
            } else {
                return '<span class="label">' + _('Stopped (Disabled)') + '</span>';
            }
        };
