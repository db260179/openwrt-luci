...
return baseclass.extend({
    ...
    load: function() {
        return Promise.all([
            ...
            uci.load('system'),
			// Mediatek SOC HW temperature reading
            L.resolveDefault(fs.read('/tmp/soc_temp'), null)
        ]);
    },
    ...
    render: function(data) {
        ...
        var temp_str = _('N/A');
        if (temp_raw !== null) {
            var val = parseInt(temp_raw.trim(), 10);
            if (!isNaN(val)) {
                // Convert millidegrees to Celsius
                var temp_c = (val > 1000) ? (val / 1000).toFixed(1) : val.toFixed(1);
                temp_str = temp_c + ' °C';
            }
        }
